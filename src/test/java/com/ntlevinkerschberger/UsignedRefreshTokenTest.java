...
@MicronautTest
public class UsignedRefreshTokenTest {
    @Inject
    @Client("/")
    HttpClient client;
    ...
    HttpClient embeddedClient = embeddedServer.getApplicationContext().createBean(HttpClient.class, embeddedServer.getURL());
    RefreshTokenGenerator embeddedRefreshTokenGenerator = embeddedServer.getApplicationContext().getBean(RefreshTokenGenerator.class);
    ...
    @Test
    void accessingSecuredURLWithoutExistingTokenReturnsUnauthorized() {
    ...
    @Test
    void accessingSecuredURLWithRevokedTokenReturnsUnauthorized() {
        Authentication user = Authentication.build("sherlock");

        String refreshToken = embeddedRefreshTokenGenerator.createKey(user);
        Optional<String> refreshTokenOptional = embeddedRefreshTokenGenerator.generate(user, refreshToken);
        assertTrue(refreshTokenOptional.isPresent());
        ...
        Argument<Map> errorArgument = Argument.of(Map.class);
        HttpClientResponseException e = assertThrows(HttpClientResponseException.class, () -> {
            embeddedClient.toBlocking().exchange(
